...
try {
  ...
  //                      key never enters the sandbox. The token is read from
  //                      the environment and is never persisted to disk.
  console.log('mounting mesa...');
  await sandbox.process.exec({
    // MESA_ACCESS_TOKEN is set to the short-lived token, NOT the private key.
    command: `MESA_ORG=${ORG} MESA_ACCESS_TOKEN=${token} mesa mount -d`,
    waitForCompletion: true,
  });
